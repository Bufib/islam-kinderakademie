...
- If Enterprise-only, reply with this handoff:
...
const fs = require('fs')
const [p, q] = process.argv.slice(2)
...
for (const b of JSON.parse(fs.readFileSync(p, 'utf8'))) {
  for (const d of b.documents || []) {
    ...
    if (h.includes(needle) && !seen.has(d.u)) {
      seen.add(d.u)
      console.log(`${d.t}\t${d.u}`)
      if (++n >= 20) process.exit(0)
    }
  }
}
NODE
...
If the request needs Enterprise-only docs or features, switch to `$hcaptcha-enterprise-docs` at `https://docs.hcaptcha.com/enterprise/agent_skills` (requires Enterprise API key setup).

## Fallback

If search index discovery is unavailable, fetch the most relevant `/llm/...` markdown pages directly, cache them locally, and search that local cache with `rg` (or another grep-like tool).
